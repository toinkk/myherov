...
RUN sudo dpkg --add-architecture i386 && wget -q "http://deb.playonlinux.com/public.gpg" -O- | sudo apt-key add - && sudo wget http://deb.playonlinux.com/playonlinux_bionic.list -O /etc/apt/sources.list.d/playonlinux.list && sudo apt-get update -y && sudo apt-get install -y playonlinux netcat snapd xterm wine-stable libgl1-mesa-glx:i386 libfreetype6:i386 libpng16-16:i386 libx11-6:i386 libxext6:i386 mousepad engrampa htop neofetch
